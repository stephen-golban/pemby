...
| D2 | Eligibility model | Tiered: green "hires from your country", yellow "likely", white "unclear", red "excluded". Every label shows its reason. Free users see green only; pass holders can opt into yellow. Red never shows. |
...
| D13 | Free vs Pass | Free: CV parse, onboarding, Brief with near misses, all matches delivered 24h late on every channel (late messages say so), 3 application kits per month. Pass: instant delivery, unlimited kits, yellow matches opt-in, the Chrome extension when it ships. |
| D14 | Trust mechanics | No-match guarantee: zero matches in any 14-day stretch of an active pass adds 14 days automatically. "Landed a role?" pauses the pass and offers a share card. The refund window is a per-provider setting: 7 days on Dodo and 14 days on Paddle, following each provider's terms and EU withdrawal rules. Until a provider is approved, the public Refund Policy states 14 days, which satisfies both. |
...
| D25 | Design | Built only with the /impeccable skill. desertant.com is a binding brand reference (research 05). Landing page is comp-first with images generated through Codex CLI; app screens are code-first. Never Higgsfield. |
...
| D27 | Growth | Private beta: referrals (both sides +14 days when the friend uploads a real CV; referrer +30 days when the friend buys), SEO country pages, public Telegram channel per country posting 24h-old green jobs. Soon after: "Landed a role?" share cards. Later: employer "hires globally" badge. |
...
| 08 | `docs/phases/08-delivery.md` | Telegram bot, email, web push, dispatcher, quiet hours | 07 | not started |
