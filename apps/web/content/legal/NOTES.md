...
| No placeholder or "coming soon" offer without usable value, Dodo prohibited item 3        | `cta.notOpen` says passes open with the beta and nobody is charged                                                                     | Partial. Passes cannot be bought yet, and the free product is not built. See section D |
...
| No ghostwriting framing                                                                           | Kits are "drafts" the user reads, edits and pastes, in `terms.md` sections 2 and 15 and `pricing/en.md` faq.kit                 | Met                                             |
...
- **Features not built yet but described.** Referrals, the referral cookie, the OpenRouter connection, tracker outcome evidence, kits and the Chrome extension are in the pages because the order asked for them. Referrals and the extension carry "when available" wording. The rest do not.
