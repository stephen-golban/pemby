 ...
 * $1 the user, $2 the freshness bar in hours.
 *
 ...
 * is counted separately because the "include the likely ones" fix only reaches yellow posts.
 *
...
const SELECT_NEAR_MISSES = `
  select blocker, total, yellow_total, job_id, title, company_name, is_demo
    from (
...
export async function loadBrief(userId: string): Promise<BriefView> {
  ...
  const profile = await client.query<ProfileRow>(
    `select residence_country, seniority::text as seniority, include_yellow, hide_no_salary, stack
  ...
  const [matches, held, nearMisses] = await Promise.all([
    client.query<MatchRow>(SELECT_MATCHES, [userId, country, FRESHNESS_HOURS]),
    client.query<HeldRow>(SELECT_HELD, [userId, FRESHNESS_HOURS]),
    client.query<NearMissRow>(SELECT_NEAR_MISSES, [userId, FRESHNESS_HOURS]),
  ]);
...
/**
 * The two one-tap fixes the near-miss groups offer (PLAN D7). `include_yellow` is a free-tier
 * setting since the D13 amendment of 2026-09-17, so nothing here checks a pass.
 */
export async function patchPreferences(
  userId: string,
  patch: PreferencesPatch,
): Promise<{ includeYellow: boolean; hideNoSalary: boolean } | null> {
  const client = getDb().$client;
  await client.query(
  ...
  const { rows } = await client.query<{ include_yellow: boolean; hide_no_salary: boolean }>(
    `update profiles
        set include_yellow = coalesce($2::boolean, include_yellow),
            hide_no_salary = coalesce($3::boolean, hide_no_salary),
            updated_at = now()
      where user_id = $1
      returning include_yellow, hide_no_salary`,
    [userId, patch.includeYellow ?? null, patch.hideNoSalary ?? null],
  );
  const row = rows[0];
  return row ? { includeYellow: row.include_yellow, hideNoSalary: row.hide_no_salary } : null;
}
