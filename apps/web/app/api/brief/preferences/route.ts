...
import type { PreferencesPatch } from "../_lib/view";
...
/**
 * `PATCH /api/brief/preferences` — the two one-tap fixes a near-miss group offers (PLAN D7):
 * include the likely (yellow) posts, and show the posts that list no pay.
 *
 * `include_yellow` has its own route rather than going through `/api/profile` because it is not
 * one of that route's writable keys, and because both fixes belong to this screen. No pass check:
 * the yellow opt-in moved to the free tier when D13 was amended on 2026-09-17.
 */
